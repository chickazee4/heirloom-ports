...
/*	from OpenSolaris "misc.cc	1.31	06/12/12"	*/
...
getmem(int size)
{
	...
	if (result == NULL) {
		...
		strcat(buf, "mksh: Fatal error: Out of memory\n");
		fputs(buf, stderr);
#ifdef SUN5_0
		exit_status = 1;
#endif
...
getname_fn(wchar_t *name, int len, Boolean dont_enter, Boolean * foundp)
{
	...
	wcstombs(mbs_name, ws.get_string(), (length * MB_LEN_MAX) + 1);
...
append_prop(Name target, Property_id type)
{
	...
	case env_mem_prop:
		...
		break;
	default:
		fatal_mksh("Internal error. Unknown prop type %d", type);
	}
...
append_string(char *from, String to, int length)
{
	...
	if (length > 0) {
		mbstowcs(to->text.p, from, length);
		to->text.p += length;
	}
	*(to->text.p) = (int) nul_char;
}
...
#else
	sigivalue = (void (*) (int)) signal(SIGINT, SIG_IGN);
	sigqvalue = (void (*) (int)) signal(SIGQUIT, SIG_IGN);
	sigtvalue = (void (*) (int)) signal(SIGTERM, SIG_IGN);
	sighvalue = (void (*) (int)) signal(SIGHUP, SIG_IGN);
#endif
	enable_interrupt(handler);
}
...
mbstowcs_with_check(wchar_t *pwcs, const char *s, size_t n)
{
	if(mbstowcs(pwcs, s, n) == -1) {
		fatal_mksh("The string `%s' is not valid in current locale", s);
	}
}
