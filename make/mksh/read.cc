...
/*	from OpenSolaris "read.cc	1.11	06/12/12"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)read.cc	1.3 (gritter) 01/13/07
 */
...
get_next_block_fn(Source source)
{
	 ...
	 */
		...
		if (length != to_read) {
			...
			if (length == 0) {
				fatal_mksh("Error reading `%s': Premature EOF",
				      mbs_buffer);
			} else {
				fatal_mksh("Error reading `%s': %s",
				      mbs_buffer,
				      errmsg(errno));
			}
		}
	...
	*source->string.text.end = 0;
	...
	if (source->inp_buf_ptr >= source->inp_buf_end) {
		if (*(source->string.text.end - 1) != (int) newline_char) {
			WCSTOMBS(mbs_buffer, file_being_read);
			warning_mksh("newline is not last character in file %s",
					     mbs_buffer);
