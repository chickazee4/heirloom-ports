...
/*	from OpenSolaris "report.cc	1.17	06/12/12"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)report.cc	1.5 (gritter) 01/13/07
 */
...
close_file(void)
{
	...
	if (err) {
		if (warning_ptr != (void (*) (char *, ...)) NULL) {
			(*warning_ptr)("Couldn't write to %s", nse_depinfo_file);
                      }
		unlink(command_output_tmpfile);
		return;
	}
...
report_dependency(char *name)
{
	...
	if (report_file == NULL) {
		...
		strcpy(target_being_reported_for, p+1);
		fputs(p+1, report_file);
		fputs(":", report_file);
		*p= ' ';
		if (p2 != NULL)
			*p2= ' ';
	...
	char	*filename;
{
	...
	sprintf(command, NOCATGETS("make %s\n"), filename);
	switch (pid= fork()) {
		case 0: /* child */
