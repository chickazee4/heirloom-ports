...
/*	from OpenSolaris "vroot.cc	1.11	06/12/12"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)vroot.cc	1.3 (gritter) 01/13/07
 */
...
void
add_dir_to_path(char *path, pathpt *pointer, int position)
{
	int		size= 0;
	int		length;
	char		*name;
...
/* Setup to remember pieces */
	...
	switch ((vp ?1:0) + (pp1 ? 2:0)) {
	    ...
	    use_name:
		strcpy(vroot_data.full_path, filename);
		vroot_data.filename_start= vroot_data.full_path;
		(*thunk)(vroot_data.full_path);
		return;
		...
		for (; vp->path != NULL; vp++) {
			...
			strcpy(vroot_data.vroot_start= p, vp->path);
			p+= vp->length;
			strcpy(vroot_data.filename_start= p, filename);
		...
		for (; pp1->path != NULL; pp1++) {
			...
			if (vroot_data.cpp_style_path) {
				strcpy(vroot_data.path_start= p, pp1->path);
				p+= pp1->length;
				*p++= '/';
			} else {
				if (pp1->length != 0) {
					strcpy(vroot_data.path_start= p,
					    pp1->path);
					p+= pp1->length;
					*p++= '/';
				};
			...
			for (; vp->path != NULL; vp++)
				for (pp= pp1, path_len= 0; pp->path != NULL; pp++, path_len++) {
					...
					if ((filename[0] == '/') || (pp->path[0] == '/')) {
						...
						strcpy(vroot_data.vroot_start= p, vp->path); p+= vp->length;
						len += vp->length;
					};
					if (vroot_data.cpp_style_path) {
						if (filename[0] != '/') {
							...
							strcpy(vroot_data.path_start= p, pp->path); p+= pp->length;
							*p++= '/';
							len += 1 + pp->length;
						};
					} else {
