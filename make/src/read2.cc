...
/*	from OpenSolaris "read2.cc	1.53	06/12/12"	*/
...
enter_member_name(wchar_t *lib_start, wchar_t *member_start, wchar_t *string_end, Name_vector current_names, Name_vector *extra_names)
{
	...
	wsncpy(buffer, lib_start, member_start - lib_start);
...
enter_percent(Name target, Chain target_group, Name_vector depes, Cmd_line command)
{
	...
	*insert = result;
	...
	if (trace_reader) {
		printf("%s:", result->name->string_mb);

		for (depe = result->dependencies; depe != NULL; depe = depe->next) {
			printf(" %s", depe->name->string_mb);
		}
...
enter_equal(Name name, Name value, Boolean append)
{
	...
	if (string[0]=='F' &&
		...
		SETVAR(temp, value, append);
/*
		fprintf(stderr, "warning: FC is obsolete, use F77 instead\n");
 */
	}
...
fatal_reader(const char * pattern, ...)
{
	...
	fflush(stdout);
	if (sun_style)
		fprintf(stderr, "%s: Fatal error in reader: ", progname);
	else
		fprintf(stderr, "%s: fatal error: ", progname);
#if 0
#ifdef DISTRIBUTED
	fprintf(stderr, "dmake: Fatal error in reader: ");
#else
	fprintf(stderr, "make: Fatal error in reader: ");
#endif
#endif
