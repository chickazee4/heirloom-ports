...
/*	from OpenSolaris "rep.cc	1.25	06/12/12"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)rep.cc	1.3 (gritter) 01/13/07
 */
...
report_recursive_init(void)
{
	...
	sprintf(nse_depinfo, "%s/%s", search_dir, NSE_DEPINFO);
	...
	while (fgetws(line, line_size, fp) != NULL) {
		...
		if (IS_WEQUALN(&colon[2], wcb,
			...
			memset((char *) rp, 0, sizeof (Recursive_make_rec));
			/*
...
report_recursive_done(void)
{
	...
	sprintf(nse_depinfo, "%s/%s", search_dir, NSE_DEPINFO);
	sprintf(tmpfile, "%s.%d", nse_depinfo, getpid());
	ofp = fopen(tmpfile, "w");
	if (ofp == NULL) {
		fprintf(stderr,
