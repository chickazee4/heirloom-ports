...
printdesc()
{
    ...
    /* The above portion is causing compile errors so I will check on this later */

	/* .DEFAULT */
	if (default_rule != NULL) {
//		printf("%s:\n", default_rule->string_mb);
	...
	/* Suffix rules */
 	for (p = hashtab.begin(), e = hashtab.end(); p != e; p++)
			if (!((p->dependency_printed) && (p->string_mb[0] == '.'))) {
				print_rule(p);
	...
	/* All other dependencies */
 	for (p = hashtab.begin(), e = hashtab.end(); p != e; p++)
			if (!(p->dependency_printed) && (p->colons != no_colon))
				print_rule(p);
	printf("\n");
	exit(0);
}
