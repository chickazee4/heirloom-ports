...
//	Boolean		make_state_locked; // Moved to lib/mksh
	...
	long int	hostid;
#ifdef LEOS
	const char	bourne_shell[] = "/Applications/SystemApplications/sh";
	const char	posix_shell[] = "/Applications/SystemApplications/sh";
#else
    const char bourne_shell[] = "/bin/sh";
    const char posix_shell[] = "/bin/sh"; /* we don't need to differentiate bourne and posix, very few people have multiple shells installed on their system and only a subset of those will have one POSIX-conformant and one POSIX-noncomnformant shell; /bin/sh is a valid assumption for the shell location on most Unix systems. */
#endif
