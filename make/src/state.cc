...
/*	from OpenSolaris "state.cc	1.27	06/12/12"	*/
...
#endif
{
	...
	if (lock_err = file_lock(make_state->string_mb, 
		...
		if (exiting) {
			sprintf(buffer, NOCATGETS("%s/.make.state.%d.XXXXXX"), tmpdir, getpid());
			report_pwd = true;
			warning("Writing to %s", buffer);
			int fdes = mkstemp(buffer);
			if ((fdes < 0) || (fd = fdopen(fdes, "w")) == NULL) {
				fprintf(stderr,
					"Could not open statefile `%s': %s",
					buffer,
					errmsg(errno));
				return;
			}
			...
			report_pwd = true;
			fatal("Can't lock .make.state");
		}
	}

	sprintf(make_state_tempfile,
	               NOCATGETS("%s.tmp"),
	               make_state->string_mb);
	/* Delete old temporary statefile (in case it exists) */
	unlink(make_state_tempfile);
	if ((fd = fopen(make_state_tempfile, "w")) == NULL) {
		...
		fatal("Could not open temporary statefile `%s': %s",
		      make_state_tempfile,
		      errmsg(lock_err));
	}
#ifdef NSE
	...
	if (attempts == 0) {
		if (unlink(make_state->string_mb) != 0 && errno != ENOENT) {
			...
			make_state_lockfile = NULL;
			make_state_locked = false;
			fatal("Could not delete old statefile `%s': %s",
			      make_state->string_mb,
			      errmsg(lock_err));
		}
		if (rename(make_state_tempfile, make_state->string_mb) != 0) {
			...
			make_state_lockfile = NULL;
			make_state_locked = false;
			fatal("Could not rename `%s' to `%s': %s",
			      make_state_tempfile,
			      make_state->string_mb,
			      errmsg(lock_err));
		}
