...
/*	from OpenSolaris "ar.cc	1.28	06/12/12"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)ar.cc	1.5 (gritter) 01/13/07
 */
...
#endif
{
	...
	mbstowcs(name_string, library->string_mb, (int) library->hash.length);
	...
	case AR_PORT:
	    for (;;) {
			...
			!IS_EQUALN(arp->ar_port.ar_fmag,
			    ...
			    fatal(
				"Read error in archive `%s': invalid archive file member header at 0x%x",
				library->string_mb,
				ftell(arp->fd)
			    );
		    }
