...
/*	from OpenSolaris "files.cc	1.37	06/12/12"	*/
...
#define d_fileno d_ino
	...
	if ((dir->hash.length > 1) || (wcb.get_string()[0] != (int) period_char)) {
		wscpy(file_name, wcb.get_string());
		MBSTOWCS(wcs_buffer, "/");
		wscat(file_name, wcs_buffer);
...
vpath_loop:
	...
	while ((dp = readdir(dir_fd)) != NULL) {
		...
		wscpy(file_name_p, tmp_wcs_buffer);
		...
		if ((dp->d_name[0] == 's') &&
			...
			wscpy(plain_file_name_p, tmp_wcs_buffer);
 ...
 */
	  ...
	  if (sccs_dir_path != NULL) {
		...
		if (file_name_p - file_name > 0) {
			...
			*file_name_p = 0;
			...
			sprintf(mb_path, NOCATGETS("%s/%s/SCCS"),
				        sccs_dir_path,
				        mbs_buffer);
			*file_name_p = tmp_wchar;
		} else {
			sprintf(mb_path, NOCATGETS("%s/SCCS"), sccs_dir_path);
		}
		MBSTOWCS(path, mb_path);
		wscpy(file_name, path);
	  } else {
		MBSTOWCS(wcs_buffer, NOCATGETS("SCCS"));
		wscpy(file_name_p, wcs_buffer);
	  }
	} else {
	...
	exists(dir = GETNAME(file_name, FIND_LENGTH));
	...
	while ((dp = readdir(dir_fd)) != NULL) {
		...
		if ((dp->d_name[0] == 's') &&
			...
			wscpy(plain_file_name_p, wcs_buffer);
			plain_file = GETNAME(plain_file_name, FIND_LENGTH);
			plain_file->stat.is_file = true;
			plain_file->stat.has_sccs = HAS_SCCS;
