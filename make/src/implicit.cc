...
/*	from OpenSolaris "implicit.cc	1.64	06/12/12"	*/
...
find_suffix_rule(Name target, Name target_body, Name target_suffix, Property *command, Boolean rechecking)
{
	...
	if (debug_level > 1) {
		printf(NOCATGETS("%*sfind_suffix_rule(%s,%s,%s)\n"),
			      recursion_level,
			      "",
 ...
 */
			...
			if ((source->stat.time > (*command)->body.line.dependency_time) &&
			    (debug_level > 1)) {
				printf("%*sDate(%s)=%s Date-dependencies(%s)=%s\n",
					      recursion_level,
					      "",
					      source->string_mb,
			...
			if (build_unconditional || out_of_date) {
				...
				if (debug_level > 0) {
					printf("%*sBuilding %s using suffix rule for %s%s because it is out of date relative to %s\n",
						      recursion_level,
						      "",
						      true_target->string_mb,
...
find_ar_suffix_rule(Name target, Name true_target, Property *command, Boolean rechecking)
{
	 ...
	 */
	if (debug_level > 1) {
		printf(NOCATGETS("%*sfind_ar_suffix_rule(%s)\n"),
			      recursion_level,
			      "",
			      true_target->string_mb);
	}
...
find_double_suffix_rule(Name target, Property *command, Boolean rechecking)
{
	...
	target_end = targ_string.get_string() + true_target->hash.length;
	if (debug_level > 1) {
		printf(NOCATGETS("%*sfind_double_suffix_rule(%s)\n"),
			      recursion_level,
			      "",
			      true_target->string_mb);
	}
...
build_suffix_list(Name target_suffix)
{
	...
	if (debug_level > 1) {
		printf(NOCATGETS("%*sbuild_suffix_list(%s) "),
			      recursion_level,
			      "",
			      target_suffix->string_mb);
	}
	...
	for (source_suffix = suffixes;
		...
		mbstowcs(rule_name + source_suffix->name->hash.length,
			      target_suffix->string_mb,
...
find_percent_rule(Name target, Property *command, Boolean rechecking)
{
	...
	if (debug_level > 1) {
		printf("%*sLooking for %% rule for %s\n",
			      recursion_level,
			      "",
			      true_target->string_mb);
	}
	...
	*/
	...
	if (debug_level > 1) {
		...
		for (pat_depe = pat_rule->dependencies;
			...
			if (depe_to_check != empty_name) {
				printf(" %s", depe_to_check->string_mb);
			}
		}
		...
		for (pat_depe = pat_rule->dependencies;
		     pat_depe != NULL;
		     pat_depe = pat_depe->next) {
			printf(" %s", pat_depe->name->string_mb);
		}
	...
	if (line->body.line.dependencies != NULL) {
		...
		for (depe = line->body.line.dependencies;
			...
			if (build_unconditional || out_of_date) {
				...
				if (debug_level > 0) {
					printf("%*sBuilding %s using pattern rule %s:",
						      recursion_level,
						      "",
						      true_target->string_mb,
						      pat_rule->name->string_mb);
					...
					printf(" because it is out of date relative to %s\n", 
						      depe->name->string_mb);
				}	
			}
		}
